...
module ItemManager
  def owned_items # 自身の所有する（自身がオーナーとなっている）全てのItemインスタンスを返します。
    Item.select{|item| item.owner == self }
  end

  def find_items_by_number_and_quantity(number, quantity) # numberと対応した自身の所有するItemインスタンスを指定されたquantitiy分返します。
